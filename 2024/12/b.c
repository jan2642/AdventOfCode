...
static int run_bfs(char * buf, int w, int h, int x, int y)
{
    ...
    while ((q = pop_head())) {
        ...
        /* Inner corners: OX.  .XO  ...  ...    (X = same char, O = other char, . = don't care)
         *                XX.  .XX  .XX  XX.
         *                ...  ...  .XO  OX.  */
        if ((val(x - 1, y - 1) != c) && (val(x, y - 1) == c) && (val(x - 1, y) == c)) corners++;
        if ((val(x + 1, y - 1) != c) && (val(x, y - 1) == c) && (val(x + 1, y) == c)) corners++;
        if ((val(x + 1, y + 1) != c) && (val(x + 1, y) == c) && (val(x, y + 1) == c)) corners++;
        if ((val(x - 1, y + 1) != c) && (val(x - 1, y) == c) && (val(x, y + 1) == c)) corners++;

        /* Outer corners: .O.  .O.  ...  ...    (X = same char, O = other char, . = don't care)
         *                OX.  .XO  .XO  OX.
         *                ...  ...  .O.  .O. */
        if ((val(x, y - 1) != c) && (val(x - 1, y) != c)) corners++;
        if ((val(x, y - 1) != c) && (val(x + 1, y) != c)) corners++;
        if ((val(x + 1, y) != c) && (val(x, y + 1) != c)) corners++;
        if ((val(x - 1, y) != c) && (val(x, y + 1) != c)) corners++;

        free(q);
    }

    printf(" %c: %d x %d = %d\n", c, area, corners, area * corners);
