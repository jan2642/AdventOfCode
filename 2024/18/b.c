...
int main(int argc, char **argv)
{
    ...
    int t;
	while(m - n > 1) { /* Binary search */
        memset(buf, '.', w * h); /* Clear the field */
		t = (n + m) / 2;         /* New target */
        for (int i = 0; i < t; i++)
            pos(bytes[i][0], bytes[i][1]) = '#';
        if (run_bfs(buf, w, h, 0, 0) > 0) /* Solution found */
            n = t; /* Continue search in upper half */
        else
            m = t; /* Continue Search in lower half */
	}
    pos(bytes[t][0], bytes[t][1]) = 'X'; /* Mark the blocking position */
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++)
            switch (pos(x,y)) {
                ...
                case 'X': printf("\033[0;31mX"); break;
            }
        printf("\033[0m\n");
    }
    
    printf("Blocking byte: %d,%d\n", bytes[t][0], bytes[t][1]);
}
